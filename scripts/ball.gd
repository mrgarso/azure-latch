...
func simulate_step(pos:= Vector3(0,0,0), vel:= Vector3(0,0,0), delta:= 0.0) -> Dictionary:
...
func _physics_process(delta: float) -> void:
	...
	var traj := predict_trajectory(global_position, velocity, 1)
	var traj_mesh = trajectory_mesh.mesh
	if traj_mesh is ArrayMesh:
		traj_mesh

func predict_trajectory(start_pos:= Vector3(0,0,0), start_vel:= Vector3(0,0,0), seconds:= 1.0, dt:= (1.0/60.0)) -> PackedVector3Array:
	...
	for i in iterations:
		var result = simulate_step(pos, vel, dt)
		pos = result.position
		vel = result.velocity
		points.append(pos)
